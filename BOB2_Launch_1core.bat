...
REM
REM  The affinity actually applied is written to BOB2_Launch_1core.log
...
powershell -NoProfile -Command ^
  "Start-Sleep -Seconds 4; $p = Get-Process Bob -ErrorAction SilentlyContinue | Select-Object -First 1; if ($p) { 'requested=0x%AFFINITY%  actual=0x{0:X}  priority={1}' -f [int64]$p.ProcessorAffinity, $p.PriorityClass } else { 'Bob.exe not found after launch' }" > "%GAMEDIR%\BOB2_Launch_1core.log" 2>&1

echo.
type "%GAMEDIR%\BOB2_Launch_1core.log"
echo.
echo ^(also saved to BOB2_Launch_1core.log^)
popd
